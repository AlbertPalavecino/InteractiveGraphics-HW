...
vec3 Shade( Material mtl, vec3 position, vec3 normal, vec3 view )
{
	...
	for ( int i=0; i<NUM_LIGHTS; ++i ) {
		...
		// Lanzar un rayo sombra
		Ray shadowRay;
		shadowRay.pos = position + normal * 1e-3; // evitar acne
		shadowRay.dir = lightDir;
		HitInfo shadowHit;
		
		if (IntersectRay(shadowHit, shadowRay) && shadowHit.t < length(lights[i].position - position)) {
			continue; // en sombra
		}
...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	hit.t = 1e30;
	bool foundHit = false;
	ray.dir = normalize(ray.dir);  // AÑADIDO aquí por seguridad global
...
vec4 RayTracer( Ray ray )
{
	...
	if ( IntersectRay( hit, ray ) ) {
		...
		for ( int bounce=0; bounce<MAX_BOUNCES; ++bounce ) {
			...
			r.pos = hit.position + hit.normal * 1e-3;  // small offset to avoid self-intersection
